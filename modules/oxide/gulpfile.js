...
gulp.task('buildDemos', function () {
  return gulp.src(['./src/demo/**/*']).pipe(gulp.dest('./build'));
});
...
gulp.task('copyTinymce', function (done) {
  if (fs.existsSync('../tinymce/js/tinymce/tinymce.min.js')) {
    ...
    console.log(
      chalk.yellow('Run yarn build in the repository root to build a local version of TinyMCE'),
    );
    const url =
      'https://cdn.tiny.cloud/1/qagffr3pkuv17a8on1afax661irst1hbr4e6tbv888sz91jc/tinymce/7-dev/tinymce.min.js';
...
gulp.task('buildSkinSwitcher', (done) => {
  ...
  const data = `uiSkins = ${JSON.stringify(uiSkins)}, contentSkins = ${JSON.stringify(
    contentSkins,
  )}`;
  const html = fs.readFileSync('./build/index.html', 'utf8');
  ...
  return gulp
    ...
    .pipe(
      through2.obj(function (file, _, cb) {
        if (file.isBuffer()) {
          const contents = `tinymce.Resource.add('${file.relative}', ${JSON.stringify(
...
gulp.task('monitor', function (done) {
  ...
  gulp
    .watch('./src/**/*')
    .on('change', gulp.series('css', 'buildDemos', 'buildSkinSwitcher', 'copyTinymce'));
});
...
gulp.task(
  'demo-build',
  gulp.series('css', 'less', 'minifyCss', 'buildDemos', 'buildSkinSwitcher'),
);
gulp.task(
  'watch',
  gulp.series('build', 'buildDemos', 'buildSkinSwitcher', 'copyTinymce', 'monitor'),
);
